-- ═══════════════════════════════════════════════════════════════════
-- 0009 — RLS para Storage bucket gestion_ganadera_fotos
--
-- Requisito: el bucket 'gestion_ganadera_fotos' debe existir en Supabase Storage
...
CREATE POLICY authenticated_storage_select ON storage.objects
  FOR SELECT TO authenticated USING (bucket_id = 'gestion_ganadera_fotos');

CREATE POLICY authenticated_storage_insert ON storage.objects
  FOR INSERT TO authenticated WITH CHECK (bucket_id = 'gestion_ganadera_fotos');

CREATE POLICY authenticated_storage_update ON storage.objects
  FOR UPDATE TO authenticated USING (bucket_id = 'gestion_ganadera_fotos') WITH CHECK (bucket_id = 'gestion_ganadera_fotos');

CREATE POLICY authenticated_storage_delete ON storage.objects
  FOR DELETE TO authenticated USING (bucket_id = 'gestion_ganadera_fotos');
